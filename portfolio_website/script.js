...
// JS handler removed to allow native form submission to PHP.
   ...
   if (projectModal && projectClose && projectTitle && projectTiles.length) {
     projectTiles.forEach(tile => {
       tile.addEventListener('click', (e) => {
          ...
          // If this is the ADMIN "Add Project" tile, let the link work normally
        if (tile.classList.contains('tile--add')) {
          return;
        }
         e.preventDefault();
         ...
         if (gh) { projectGithub.href = gh; projectGithub.style.display = ''; }
          ...
          // NEW: wire Edit/Delete in the modal (ADMIN only elements will exist)
          const id = tile.dataset.id || '';
          const delInput = document.getElementById('deleteProjectId');
          if (delInput) delInput.value = id;
          const editLink = document.getElementById('projectEditLink');
          if (editLink) editLink.href = '/portfolio/Admin/projects/edit.php?id=' + encodeURIComponent(id);
...
document.addEventListener('DOMContentLoaded', function(){
  var toast = document.querySelector('#contact .contact-flash');
  if (!toast) return;

  // 1) Drop ?sent=… from the URL so reloads won’t show it again
  try {
    ...
    // Fallback (older browsers): remove all query params
    history.replaceState({}, '', window.location.pathname + window.location.hash);
  }

  // 2) Keep your fade-out timing (you currently use 4000ms)
  setTimeout(function(){
    toast.classList.add('is-fading');   // triggers CSS transition
    setTimeout(function(){
